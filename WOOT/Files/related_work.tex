...
attack still requires physical access to the victim's device and is unable to cause more
...
victim's device, they could be recognized as a keyboard or mouse. Then attackers can
...
attacks~\cite{badusb} are unable to obtain video feedback from the victim's device
...
\ac{JFA}~\cite{JFC},
abuses this standard and monitors the victim's device utilizing \ac{USB} 2.0 protocol. Although \ac{JFA}~\cite{JFC} could exfiltrates video data from the victim's device without permission,  
there are two main limitations compared with \tool: 
(1) \ac{JFA}~\cite{JFC} can only exfiltrate video data passively from the victim's device, while \tool can control the victim's device at the same time as monitoring the screen. The data can only be collected by \ac{JFA} after the victim's interaction, while \tool can gather sensitive data actively through \ac{HID} injections. For example, if a victim leaves his/her phone charging without locking it, \ac{JFA} cannot conduct attacks in this case, while \tool can actively control his/her device and gather sensitive information.
(2) The official list of \ac{MHL} Devices~\cite{MHLlist} shows that the latest mobile devices supporting \ac{MHL} were released in 2015, which indicates the lack of support of \ac{MHL} and limits the attack range of \ac{JFA}. \tool attacks the victim's device over \ac{USB} Type-C interface, the latest \ac{USB} connector designed to replace all previous USB connectors including MicroUSB~\cite{li2018usb}. So \tool has a wider attack range. 
...
will stop all \ac{USB} functions other than charging.

Under the premise of ensuring the full functionality of \ac{USB} devices, some works
improve the security while establishing a connection. Windows Defender
...
by the \ac{USB} device to the user and lets the user decide whether to authorize. When a device
...
Moreover, Mueller \emph{et al.}~\cite{MuellerZN19} improve the security without changing the user experience. It can detect the presence of the user, block malicious devices in the user's absence until the session is unlocked.
...
effective because the attacker cannot obtain the screen of the victim's device using
...
standard to steal the video stream from the victim's mobile phone. In order to mitigate this
...
achieve almost absolute security, the functionality of \ac{USB} is sacrificed.
