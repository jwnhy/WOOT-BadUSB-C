...
(1) \ac{JFA}~\cite{JFC} can only exfiltrate video data passively from the victim's device, while \tool can control the victim's device at the same time as monitoring the screen. All data can be collected by \ac{JFA} only after the victim's interaction, while \tool can gather sensitive data actively through \ac{HID} injections. For example, if a victim leaves his/her phone charging without locking it, \ac{JFA} cannot conduct attacks in this case, while \tool can actively control his/her device and gather sensitive information.
