...
Juice Filming Attacks(JFA)~\cite{JFC},
abuses this standard and monitoring the victim's device utilizing \ac{USB} 2.0 protocol. Although the JFA~\cite{JFC} could exfiltrates video data from the victim's device without permission,  
there are two main limitation compared with \tool: 
\ding{182} JFA~\cite{JFC} can only exfiltrate video data passively from the victim's device, while \tool can control the victim's device at the same time as monitoring the screen. All data can be collected by JFA only after the victim's interaction, while \tool can gather sensitive data actively through \ac{HID} injection. For example, if a victim leaves his/her phone charging without locking it, JFA cannot obtain anything in this case, while \tool can actively control his/her device and gather sensitive information.
