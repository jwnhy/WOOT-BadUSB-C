...
could not be performed due to a lack of \ac{UI} status. Whatever \acp{HID} the
...
implements an authorization procedure when a new device is plugged in and blocks other functions except authorized ones.
...
send constructed \ac{HID} packets to the victim. These constructed \ac{HID} packets are
interpreted by the victim as valid keystrokes and mouse movements. Thus, attackers can execute arbitrary scripts on the victim's device. This is the
function of BadUSB. Based on this, we made the following
improvements.

First, as \tool has established a feedback channel using \ac{USB} 3.x protocol, when performing keystroke and mouse movements injection, an attacker can obtain real-time feedback from the victim's device. This feedback allows an attacker to tell whether the previous attack succeeded and decide what to perform next. Existence of such a feedback channel largely strengthens the attack ability of \tool.
...
Also, with mouse
supported, \tool extends the original BadUSB attack and results in further attacks.
...
through GSM/WiFi and replays them to the victim's device by the \ac{USB} emulation.
