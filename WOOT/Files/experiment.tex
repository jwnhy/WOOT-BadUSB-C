...
\begin{itemize}
	\item \textbf{Screen Mirror}:
		During our experiment, we noticed that some devices do not mirror their screen to our \tool by default. In this case, \tool will inject a sequence of keystrokes to set the victim's device into mirror their screen. In both Windows 10 and Ubuntu (Gnome Desktop), \tool can inject `Win+P' multiple times to switch between different modes for external display. In MacOS, according to the official manual~\cite{appleman}, \tool can inject \mbox{`Command+F1'} keystroke to  set the monitor into mirroring the primary screen. However, if the victim has default  setting for external monitor display mode, \tool will have to try these keystroke repeatedly to set itself into mirroring the victim's screen. For example, in Ubuntu, if the default setting is to disable external screen, \tool needs to press `Win+P' four times while if the default setting is to extend the primary screen, \tool needs to press `Win+P' five times.
...
smartphone in EMUI 11.0.0.135(C00E128R2P5) \mbox{(Android 10.0 based)} with a \ac{USB} Type-C interface, as the
