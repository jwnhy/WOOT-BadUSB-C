...
\textit{Setup.} \fengwei{I would suggest to add references for devices such as
Pi, ATMEGA32U4 board, Yamazawa, etc.} As mentioned in Section~\ref{sec:badusb},
...
prototype. To begin with, we choose the Raspberry Pi 4B as the embedded
computer inside \tool, which is powerful enough to process video data and has
onboard WiFi chip. As for the HID emulator, we decide to use a ATMEGA32U4 board
...
Yamazawa, which supports HDMI, USB 2.0, and many other exported peripheral.
...
understand.} our \tool can already perform limited keystroke to complete the
...
the video stream. Don't understand why it is more power-efficient.} After the
...
For an example, Brick\fengwei{reference?} is such a power bank sharing service
...
QR code payment is a new type of payment method that are popular in Asia. Its most well known cases are WeChat Pay and Alipay\fengwei{reference?}. QR code payment provides merchant and client a convenient way of offline payment while ensuring equivalent security as the credit card.
