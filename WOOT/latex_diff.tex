...
%DIF DEL /tmp/N4FUPx3z7t/latexdiff-vc-314f235ab46b938161be51ed502ce917872bfd7b/BadUSBC.tex   Mon Mar 22 08:55:03 2021
...
We surveyed related works on \ac{USB} attacks in Section~\ref{subsec:usb_attack} and
...
execute malicious payloads as they are using the victim's device. Due to the
...
\ac{MHL} extends the \ac{USB} standard and allows the video signal to
be transmitted through the \ac{USB} interface even for \ac{USB} 2.0 protocol. 
\ac{JFA}~\cite{JFC},
abuses this standard and monitoring the victim's device utilizing \ac{USB} 2.0 protocol. Although \ac{JFA}~\cite{JFC} could exfiltrates video data from the victim's device without permission,  
there are two main limitation compared with \tool: 
\ding{182} \ac{JFA}~\cite{JFC} can only exfiltrate video data passively from the victim's device, while \tool can control the victim's device at the same time as monitoring the screen. All data can be collected by \ac{JFA} only after the victim's interaction, while \tool can gather sensitive data actively through \ac{HID} injection. For example, if a victim leaves his/her phone charging without locking it, \ac{JFA} cannot obtain anything in this case, while \tool can actively control his/her device and gather sensitive information.
\ding{183} The official list of \ac{MHL} Devices~\cite{MHLlist} shows that the latest mobile devices supporting \ac{MHL} were released in 2015, which indicates the lack of support of \ac{MHL} and limits the attack range of \ac{JFA}. \tool attacks over \ac{USB} Type-C interface, the latest \ac{USB} connector designed to replace all previous USB connector including MicroUSB~\cite{li2018usb}. So \tool has a wider attack range. 

% Using USB devices to inject code
Besides attacking from the protocol perspective, previous works\shuqing{re?} use a
...
From the hardware perspective, the BadUSB attack requires \mbox{(D+ \& D-)} pins which
...
channels by adding a blocker in the connector. This blocker can cut off the \mbox{(D+ \& D-)} connection while leaving the power pins intact. However, this method poses
...
effective because the attacker cannot obtain the screen of the victim host using
...
exists an attack called \ac{JFA}~\cite{JFC} which abuses the \ac{MHL}
...
like GPU/CPU usage to detect \ac{JFA} attacks.
...
pretend to be normal \ac{HID} devices and utilize the \ac{USB} protocol to perform attacks.
...
attacks. However, we are still unable to bypass defenses like GoodUSB~\cite{tian2015defending}, which completely blocks unauthorized functionality. This will be further discussed in Section~\ref{sec:discussion}
...
\begin{itemize}
	...
	\item\underline{Single Board Computer} processes the video stream from the victim via the Video Capture Card or sends commands to the victim via \ac{HID} Emulator.
...
interpreted by the victim as valid keystrokes and mouse movements. Thus, \DIFdelbegin \DIFdel{the
...
and output its video stream. This stream is later processed by the embedded
...
payment code detector for the proof-of-concept purpose, which demonstrates that we successfully transfered money from a victim to an
...
\begin{itemize}
	\item \textbf{Screen Mirror}:
		During our experiment, we noticed that some devices did not mirror their screens to our \tool by default. In this case, \tool would inject a sequence of keystrokes to set the victim's device \DIFdelbegin \DIFdel{into mirror their screens}\DIFdelend \DIFaddbegin \DIFadd{to mirror the primary screen}\DIFaddend . In both Windows 10 and Ubuntu (Gnome Desktop), \tool can inject \DIFdelbegin \DIFdel{`}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend Win+P\DIFdelbegin \DIFdel{' (`Super' is another name for the `Win' key }\DIFdelend \DIFaddbegin \DIFadd{'' (The ``Win'' key is the ``Super'' key in Windows and Ubuntu by default}\DIFaddend ) to switch between different modes for external display. Figure~\ref{fig:ubuntu_switch} illustrates how these keystrokes works on Ubuntu and \DIFdelbegin \DIFdel{Windows 10 is similar . }\DIFdelend \DIFaddbegin \DIFadd{it is similar in Windows 10. }\DIFaddend In MacOS, according to the official manual~\cite{appleman}, \tool can inject \mbox{\DIFdelbegin \DIFdel{`}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend Command+F1\DIFdelbegin \DIFdel{'}\DIFdelend \DIFaddbegin \DIFadd{''}\DIFaddend } keystrokes to set the \DIFdelbegin \DIFdel{monitor into mirroring }\DIFdelend \DIFaddbegin \DIFadd{victim's device to mirror }\DIFaddend the primary screen. In EMUI, there is a special mode called \DIFdelbegin \DIFdel{`Desktop Mode'}\DIFdelend \DIFaddbegin \DIFadd{``Desktop Mode''}\DIFaddend , which allows users to use their smartphones like a desktop computer with an external screen. If this mode is \DIFaddbegin \DIFadd{enabled, as }\DIFaddend victim's \DIFdelbegin \DIFdel{default setting}\DIFdelend \DIFaddbegin \DIFadd{``desktop'' can be obtained}\DIFaddend , \tool can inject mouse movements to switch to mirror mode\DIFdelbegin \DIFdel{as }%DIFDELCMD < \tool %%%
\DIFdel{can obtain victim's `desktop' in this mode}\DIFdelend .
		\begin{figure}[H]
			\includegraphics[width=\linewidth]{./Figs/ubuntu_switch.png}\\
			\caption{\DIFaddbeginFL \DIFaddFL{Switching }\DIFaddendFL Screen Mode \DIFdelbeginFL \DIFdelFL{Switch }\DIFdelendFL in Ubuntu.}
...
\DIFdelbegin \DIFdel{To further test }\DIFdelend \DIFaddbegin \DIFadd{We conducted experiments to further validate }\DIFaddend the capabilities of \tool 
in \DIFdelbegin \DIFdel{this attack scenario , we conducted a simulation experiment.
We selected }\DIFdelend \DIFaddbegin \DIFadd{the attack scenario introduced in Section~\ref{subsec:attack-scenario}.
}\DIFaddend 11 applications \DIFaddbegin \DIFadd{were selected and tested }\DIFaddend on \mbox{\textit{HUAWEI P30 (ELE-AL00)}} \DIFdelbegin \DIFdel{, test each application with the following strategy
}%DIFDELCMD < \ding{182} %%%
\DIFdelend \DIFaddbegin \DIFadd{step by step:
(1) }\DIFaddend Login in with a test account.
\DIFdelbegin %DIFDELCMD < \ding{183} %%%
\DIFdelend \DIFaddbegin \DIFadd{(2) }\DIFaddend Keep the default settings.
\DIFdelbegin %DIFDELCMD < \ding{184} %%%
\DIFdelend \DIFaddbegin \DIFadd{(3) }\DIFaddend Attach attacker's \tool to the test device.
\DIFdelbegin %DIFDELCMD < \ding{185} %%%
\DIFdelend \DIFaddbegin \DIFadd{(4) }\DIFaddend Simulate victim's daily usage of the application.
...
We are very grateful to our shepherd, Michael Schwarz\DIFaddbegin \DIFadd{, }\DIFaddend and the anonymous reviewers for their valuable \DIFdelbegin \DIFdel{comments }\DIFdelend \DIFaddbegin \DIFadd{feedback }\DIFaddend that improved the paper.
