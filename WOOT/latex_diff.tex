...
%DIF DEL /tmp/O_sVVMfwcY/latexdiff-vc-34a97a8d8868555507dd875f30368a64cb8b20eb/WOOT/BadUSBC.tex   Fri Mar 12 15:55:21 2021
%DIF ADD WOOT/BadUSBC.tex                                                                         Fri Mar 19 13:11:13 2021
...
\DIFdelbegin \DIFdel{Since }\DIFdelend \DIFaddbegin \DIFadd{Although many smartphones equipped with USB-C connectors do not support }\DIFaddend \ac{USB} \DIFdelbegin \DIFdel{Type-C }\DIFdelend \DIFaddbegin \DIFadd{3.x protocol, such as the products of Xiaomi, more and more vendors like HUAWEI and Samsung tend to support }\ac{USB} \DIFadd{3.x protocol in their \mbox{high-end} smartphones~\mbox{%DIFAUXCMD
...
be transmitted through the \ac{USB} interface \DIFaddbegin \DIFadd{even for }\ac{USB} \DIFadd{2.0 protocol}\DIFaddend . 
JFA~\cite{JFC}, short for Juice Filming Attacks\DIFdelbegin \DIFdel{is an example that }\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend abuses this standard and \DIFaddbegin \DIFadd{monitoring the victim's device utilizing }\ac{USB} \DIFadd{2.0 protocol. Although the JFA~\mbox{%DIFAUXCMD
...
in JFA}\DIFdelend \DIFaddbegin \DIFadd{'s device without permission}\DIFaddend ,  
\DIFdelbegin \DIFdel{this data exfiltration is not
combined with BadUSB attacks, and }\DIFdelend \DIFaddbegin \DIFadd{there are two main limitation compared with }\tool\DIFadd{: 
}\ding{182} \DIFadd{JFA~\mbox{%DIFAUXCMD
\cite{JFC} }\hspace{0pt}%DIFAUXCMD
can only exfiltrate video data passively from the victim's device, while }\tool \DIFadd{can control the victim's device at the same time as monitoring the screen. All data can be collected by JFA only after the victim's interaction, while }\tool \DIFadd{can gather sensitive data actively through }\ac{HID} \DIFadd{injection. For example, if the victim leaves his/her phone charging without locking it, JFA cannot obtain anything in this case, while }\tool \DIFadd{can actively control his/her device and gather sensitive information.
...
}\DIFdelend UGREEN~\cite{ugreen}, which supports HDMI, \ac{USB} 2.0, and many other exported peripherals.
...
\begin{itemize}
	\item \textbf{Screen Mirror}:
		During our experiment, we noticed that some devices \DIFdelbegin \DIFdel{do }\DIFdelend \DIFaddbegin \DIFadd{did }\DIFaddend not mirror their \DIFdelbegin \DIFdel{screen }\DIFdelend \DIFaddbegin \DIFadd{screens }\DIFaddend to our \tool by default. In this case, \tool \DIFdelbegin \DIFdel{will }\DIFdelend \DIFaddbegin \DIFadd{would }\DIFaddend inject a sequence of keystrokes \DIFdelbegin \DIFdel{and mouse movements }\DIFdelend to set the victim's device into mirror their \DIFdelbegin \DIFdel{screen. For example, in }\DIFdelend \DIFaddbegin \DIFadd{screens. In both }\DIFaddend Windows 10 \DIFaddbegin \DIFadd{and Ubuntu (Gnome Desktop)}\DIFaddend , \tool can inject `Win+P' \DIFdelbegin \DIFdel{keystroke and a mouse click on the `Duplicate' option. However, as }\DIFdelend \DIFaddbegin \DIFadd{(`Super' is another name for the `Win' key) to switch between different modes for external display. Figure~\ref{fig:ubuntu_switch} illustrates how these keystrokes works on Ubuntu and Windows 10 is similar. In MacOS, according to the official manual~\mbox{%DIFAUXCMD
\cite{appleman}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend \tool \DIFdelbegin \DIFdel{is unable to obtain the screen during this process, this can only be performed blindly with a possibility of failure.
		}\DIFdelend \DIFaddbegin \DIFadd{can inject \mbox{`Command+F1'} keystroke to set the monitor into mirroring the primary screen. In EMUI, there is a special mode called `Desktop Mode', which allows users to use their smartphones like a desktop computer with an external screen. If this mode is victim's default setting, }\tool \DIFadd{can inject mouse movements to switch to mirror mode as }\tool \DIFadd{can obtain victim's `desktop' in this mode.
		}\begin{figure}[H]
			\includegraphics[width=\linewidth]{./Figs/ubuntu_switch.png}\\
			\caption{\DIFaddFL{Screen Mode Switch in Ubuntu.}}
...
\DIFadd{To further }\DIFaddend validate our attacking scenario, we invited 10 volunteers, who are students in our university, to participate and conduct a user study.
...
\DIFaddbegin \begin{table*}[t]
	...
	\begin{tabular}{|l|l|l|l|}
		\hline
		\textbf{\DIFaddFL{Keyword}} & \textbf{\DIFaddFL{Text}}                                                             & \textbf{\DIFaddFL{Name}}                                   & \textbf{\DIFaddFL{Frame Number}} \\ \hline
		\DIFaddFL{username                          }& \DIFaddFL{X 8B cas.******.edu.cn Username: 11****18 Password:                                        }& \DIFaddFL{\textless{}user1\textgreater{} }& \DIFaddFL{385                                    }\\ \hline
		\DIFaddFL{username                          }& \DIFaddFL{Login Weibo Login with SMS and verification code ...... +86 151****4587                    }& \DIFaddFL{\textless{}user5\textgreater{} }& \DIFaddFL{1947                                   }\\ \hline
		\DIFaddFL{username                          }& \DIFaddFL{QQ 14*****50| Login with phone number New user registration 2345678 9 0                    }& \DIFaddFL{\textless{}user3\textgreater{} }& \DIFaddFL{4308                                   }\\ \hline
		\DIFaddFL{username                          }& \DIFaddFL{connect to *** username h*****l Save account information Open VPN.....                     }& \DIFaddFL{\textless{}user6\textgreater{} }& \DIFaddFL{7925                                   }\\ \hline
		\DIFaddFL{phone number                      }& \DIFaddFL{Login with phone number ...... +86 186****2483 |                                           }& \DIFaddFL{\textless{}user1\textgreater{} }& \DIFaddFL{313                                    }\\ \hline
		\DIFaddFL{phone number                      }& \DIFaddFL{Log in with your mobile phone number. ...... mobile phone number 131****9310 }& \DIFaddFL{\textless{}user9\textgreater{} }& \DIFaddFL{210                                    }\\ \hline
		\DIFaddFL{@                                 }& \DIFaddFL{contact email 30*******7G@qq.com, contact phone 027-88******                               }& \DIFaddFL{\textless{}user7\textgreater{} }& \DIFaddFL{5324                                   }\\ \hline
		\DIFaddFL{@                                 }& \DIFaddFL{Account 73*****5@qq.com @qq.com @163.com @gmail.com 	......      }& \DIFaddFL{\textless{}user6\textgreater{} }& \DIFaddFL{621                                    }\\ \hline
	\end{tabular}
	\linebreak
	\caption{\DIFaddFL{Examples of Searching }\ac{OCR} \DIFaddFL{Results with Keywords.}}
...
\begin{table*}[t]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\hline
		\textbf{\DIFaddFL{Keyword}}                               & \DIFaddFL{Account }& \DIFaddFL{Password }& \DIFaddFL{Phone number }& \DIFaddFL{Email }& \DIFaddFL{Username }& \DIFaddFL{Regexp for email~  }\\
		\hline
		\textbf{\DIFaddFL{Number of records containing keywords}} & \DIFaddFL{680     }& \DIFaddFL{1,596     }& \DIFaddFL{1,510         }& \DIFaddFL{164   }& \DIFaddFL{522      }& \DIFaddFL{78                 }\\
		\hline
		\textbf{\DIFaddFL{Total Number of Records}} & \multicolumn{6}{c|}{4,172} \\
		\hline
	\end{tabular}
	\linebreak
	\caption{\DIFaddFL{Privacy Extraction Result.}}
	\label{table:information_extracted}
\end{table*}
...
%DIFDELCMD < %%%
\DIFdelend Responsible disclosure have already been carried out, we have already contacted \DIFdelbegin \DIFdel{Huawei }\DIFdelend \DIFaddbegin \DIFadd{HUAWEI }\DIFaddend and Apple through proper channel\DIFdelbegin \DIFdel{and are waiting for mitigation being deployed. We revived response from Huawei }\DIFdelend \DIFaddbegin \DIFadd{. We received response from HUAWEI }\DIFaddend on \nth{7} March and discussed about the mitigation plan on \nth{10} March while Apple has not responded yet. \DIFaddbegin \DIFadd{HUAWEI security team has confirmed this issue and been weighing different mitigation approaches. We are actively facilitating their mitigating process and waiting for mitigation to be deployed. After the mitigation is deployed, HUAWEI will apply a CVE ID for this vulnerability.
